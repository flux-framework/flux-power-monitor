...
static void timer_handler (flux_reactor_t *r, flux_watcher_t *w, int revents, void *arg)
{
    ...
    if (ret == 0) {
        ...
        if (power_data == NULL) {
            flux_log_error (h,
                            "%s: Error in Creating Node Power Info Object",
                            __FUNCTION__);
            return;
        }
        ...
        if (per_node_retro_queue_buffer == NULL) {
            ...
            if (per_node_retro_queue_buffer == NULL) {
                flux_log_error (h,
                                "%s: Error in Creating Root Node Power Data",
                                __FUNCTION__);
                return;
            }
        }
...
response_power_data *get_response_power_data (flux_t *h,
                                              const char *hostname,
                                              uint64_t start_time,
                                              uint64_t end_time)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (strcmp (hostname_list[i], hostname) == 0) {
            if (i > 0) {
                flux_future_t *f = flux_rpc_pack (h,
                                                  "flux_pwr_monitor.request_power_data_"
                                                  "from_node",
                                                  i,
                                                  FLUX_RPC_STREAMING,
                                                  "{s:I s:I s:s}",
                ...
                if (flux_rpc_get_unpack (f,
                    ...
                    < 0) {
                    flux_log_error (h,
                                    "error in Unpacking request from children nodes");
                    return NULL;
                }
...
error:
    if (flux_respond_error (h,
                            NULL,
                            errno,
                            "Error:unable to unpack flux_power_monitor.collect_power ")
        < 0)
...
void flux_pwr_monitor_get_node_power (flux_t *h,
    ...
    if (rank == 0) {
        ...
        if (flux_request_unpack (msg,
            ...
            if (flux_respond_error (h,
                                    msg,
                                    errno,
                                    "error Unpacking get_node_power request from "
                                    "client")
                < 0)
                flux_log_error (h,
        ...
        json_array_foreach (node_list, index, node_hostname)
        {
            ...
            if (hostname == NULL) {
                flux_log_error (h,
                                "JSON parse error for the hostname at index %ld",
                                index);
            } else {
                ...
                if (power_data == NULL) {
                    flux_log_error (h,
                                    "Error:  Unable to get agg power_data hostname:%s",
                                    hostname);
                } else {
        ...
        for (int i = 0; i < num_nodes_data_present; i++) {
            json_t *data_obj = json_pack ("{s:s s:s s:{s:f s:f s:f s:f s:I s:I}}",
                                          "hostname",
                                          power_data_nodes[i]->hostname,
                                          "data_presence",
                                          get_data_presence_string (
                                              power_data_nodes[i]->data_presence),
                                          "node_power_data",
                                          "node_power",
                                          power_data_nodes[i]->agg_node_power,
                                          "cpu_power",
                                          power_data_nodes[i]->agg_cpu_power,
                                          "gpu_power",
                                          power_data_nodes[i]->agg_gpu_power,
                                          "mem_power",
                                          power_data_nodes[i]->agg_mem_power,
                                          "result_start_time",
                                          power_data_nodes[i]->start_time,
                                          "result_end_time",
                                                    ...
                                                    void *arg)
{
    ...
    if (flux_request_unpack (msg,
        ...
        if (flux_respond_error (h,
                                msg,
                                errno,
                                "error responding to flux_pwr_montior.get_node_power "
                                "request")
            < 0)
            flux_log_error (h,
                            "error responding to flux_pwr_montior.get_node_power "
    ...
    response_power_data *power_data = get_agg_power_data (per_node_retro_queue_buffer,
                                                          node_hostname,
                                                          start_time,
                                                          end_time);
    if (power_data == NULL) {
        flux_log_error (h,
                        "Unable to get agg data from node for "
    ...
    if (flux_respond_pack (h,
                           ...
                           "{s:f s:f s:f s:f s:I s:I s:i}",
                           "n_p",
                           power_data->agg_node_power,
                           "c_p",
                           power_data->agg_cpu_power,
                           "g_p",
                           power_data->agg_gpu_power,
                           "m_p",
                           power_data->agg_mem_power,
                           "r_stime",
                           power_data->start_time,
                           "r_etime",
                           power_data->end_time,
                           "d_p",
                           (int)power_data->data_presence)
        < 0) {
...
void flux_pwr_monitor_get_hostname (flux_t *h,
                                    flux_msg_handler_t *mh,
                                    const flux_msg_t *msg,
                                    void *arg)
{
...
static const struct flux_msg_handler_spec htab[] = {
    ...
    {FLUX_MSGTYPE_REQUEST,
     "flux_pwr_monitor.get_hostname",
     flux_pwr_monitor_get_hostname,
     0},
    {FLUX_MSGTYPE_REQUEST,
     "flux_pwr_monitor.get_node_power",
     flux_pwr_monitor_get_node_power,
     0},
    {FLUX_MSGTYPE_REQUEST,
     "flux_pwr_monitor.request_power_data_from_node",
     flux_pwr_monitor_request_power_data_from_node,
     0},
...
int mod_main (flux_t *h, int argc, char **argv)
{
    ...
    flux_rpc_pack (h,                                // flux_t *h
                   ...
                   "hostname",
                   node_hostname);
    flux_watcher_t *timer_watch_p = flux_timer_watcher_create (flux_get_reactor (h),
                                                               1.0,
                                                               sampling_rate,
                                                               timer_handler,
                                                               h);
