...
json_t *get_response_power_data (flux_t *h,
                                 const char *hostname,
                                 uint64_t start_time,
                                 uint64_t end_time)
{
        ...
        if (strcmp (hostname_list[i], hostname) == 0) {
            if (i > 0) {
                ...
                response_power_data *power_data =
                    get_agg_power_data (per_node_retro_queue_buffer,
                                        node_hostname,
                                        start_time,
                                        end_time);
                if (power_data == NULL) {
                                      ...
                                      void *arg)
{
    if (rank == 0) {
        ...
        for (size_t i = 0; i < node_list_size; i++) {
            ...
            if (!data_obj) {
                log_error ("Failed to create JSON object for power data of hostname:%s",
                           hostname);
                continue;
            }
                                                    ...
                                                    void *arg)
{
    ...
    if (flux_request_unpack (msg,
        ...
        < 0) {
        log_error ("error responding to flux_pwr_montior.get_node_power request");
        if (flux_respond_error (h,
                                msg,
                                errno,
    ...
    if (data == NULL) {
        log_error (
            "Unable to convert power Data to JSON "
    ...
    if (flux_respond_pack (h, msg, "{s:O}", "data", data) < 0) {
        json_decref (data);
        response_power_data_destroy (power_data);
        log_error ("Error in RPC:flux_pwr_monitor_request_power_data_from_node");
        return;
    }
    response_power_data_destroy (power_data);
    json_decref (data);
