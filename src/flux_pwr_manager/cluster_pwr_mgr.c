...
double redistribute_power (cluster_mgr_t *cluster_mgr, int num_of_devices)
{
    ...
    while (data != NULL) {
        ...
        double new_job_powelimit = new_per_device_powerlimit * job_data->num_of_gpus;
...
int get_new_job_device_info (flux_t *h,
                             uint64_t job_id,
                             size_t num_of_nodes,
                             node_device_info_t ***node_data)
{
    ...
    if (json_r) {
        if (cluster_self_ref) {
            int length = 0;
            if (update_device_info_from_json (json_r, node_data, &length, num_of_nodes)
                < 0) {
                return -1;
            }
        } else
            log_error (
                "cluster_self_ref of job_pwr_mgr does not exist when parsing "
                "device info");
    } else {
                             ...
                             char *job_name)
{
    ...
    if (get_new_job_device_info (flux_handle, jobId, requested_nodes_count, &node_data)
        < 0) {
