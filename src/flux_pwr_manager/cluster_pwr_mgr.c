...
double redistribute_power (cluster_mgr_t *cluster_mgr, int num_of_devices)
{
    ...
    while (data != NULL) {
        ...
        double new_job_powelimit =
            new_per_device_powerlimit * job_mgr_next->num_of_gpus;
...
int get_new_job_device_info (flux_t *h, uint64_t job_id, node_device_info_t *node_data)
{
    if (job_id == 0 || h == NULL)
        return -1;
    flux_future_t *f;
    if (!(f = flux_rpc_pack (h,
                             ...
                             "flags",
                             0))) {
        log_error ("ROC_ERROR:Unable to send RPC to get device info for the job %s",
    ...
    if (cluster_self_ref)
        update_device_info_from_json (json_r, &node_data, &num_of_nodes);
    else
        log_error (
            "cluster_self_ref of job_pwr_mgr does not exist when parsing device info");
    flux_future_destroy (f);
                               ...
                               node_device_info_t *node_data)
{
    ...
    // Each job is getting total_pow/total_num_of_nodes.
        ...
        double new_per_device_powerlimit =
            redistribute_power (cluster_mgr,
...
int cluster_mgr_add_new_job (cluster_mgr_t *cluster_mgr,
                             uint64_t jobId,
                             char **nodelist,
                             int requested_nodes_count,
                             char *cwd,
                             char *job_name)
{
    if (jobId == 0 || !nodelist || !cwd || !cluster_mgr || requested_nodes_count == 0)
        return -1;
...
int cluster_mgr_remove_job (cluster_mgr_t *cluster_mgr, uint64_t jobId)
{
    ...
    if (!job_map) {
        ...
        log_message ("hash table first_element %ld",
                     (uint64_t)zlistx_first (
                         zhashx_keys (cluster_mgr->job_hash_table)));
        return -1;
    }
                                           ...
                                           void *args)
{
    ...
    redistribute_power (cluster_self_ref, current_nodes_utilized);
}
