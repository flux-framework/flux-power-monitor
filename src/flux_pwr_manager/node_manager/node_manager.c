...
int node_manager_init (flux_t *h,
                       uint32_t rank,
                       uint32_t size,
                       char *hostname,
                       size_t buffer_size,
                       size_t sampling_rate)
{
    ...
    retro_queue_buffer_t *buffer = retro_queue_buffer_new (power_buffer_size, free);
    if (buffer == NULL)
        return -1;
...
int node_manager_update_and_set_powercap (node_job_info *job,
                                          double powercap,
                                          int deviceId)
{
    if (job == NULL)
...
{
    log_message ("FFT state %d enable_dynamic_powercapping %d",
                 fft_enable,
    ...
    if (enable_dynamic_powercapping) {
        ...
        while (job_data != NULL) {
            for (int i = 0; i < job_data->num_of_devices; i++) {
                ...
                double new_powecap =
                    policy->get_powercap (policy,
                                          job_data->powerlimit[job_data->deviceId[i]],
                                          job_device_current_powercap,
                                          job_data->external_power_data_reference
                                              [job_data->deviceId[i]]);
                // log_message("getting new powercap");
                if (node_manager_update_and_set_powercap (job_data,
                                                          new_powecap,
...
int node_manager_finish_job (uint64_t jobId)
{
    ...
    file_logger_add_data_to_buffer (file_log,
                                    j_key,
                                    strlen (j_key),
                                    j_data,
                                    strlen (j_data));
...
void node_manager_set_pl_cb (flux_t *h,
                             ...
                             void *args)
{
    ...
    node_device_info_t *device_data =
        json_to_node_device_info (device_data_json, &powerlimit_data, &powerlimit_size);
    ...
    for (int k = 0; k < device_data->num_of_gpus; k++) {
        ...
        file_logger_add_data_to_buffer (file_log,
                                        ...
                                        j_data,
                                        strlen (j_data));
        node_manager_set_powerlimit (jobId,
                                     powerlimit_data[k],
                                     device_data->device_id_gpus[k]);
    }
}

void node_manager_end_job_cb (flux_t *h,
                              flux_msg_handler_t *mh,
                              const flux_msg_t *msg,
                              void *args)
{
...
void node_manager_new_job_cb (flux_t *h,
                              flux_msg_handler_t *mh,
                              const flux_msg_t *msg,
                              void *args)
{
    ...
    for (int i = 0; i < powerlimit_size; i++) {
        ...
        snprintf (j_key,
                  LOG_LEN,
                  "jobid_%lu_pl_new_job_gpu_%d",
                  jobId,
                  device_data->device_id_gpus[i]);
        ...
        file_logger_add_data_to_buffer (file_log,
                                        ...
                                        j_data,
                                        strlen (j_data));
        if ((node_manager_set_powerlimit (jobId,
                                          powerlimit_data[i],
                                          device_data->device_id_gpus[i])
             < 0))
...
void node_manager_disable_pm_cb (flux_t *h,
                                 flux_msg_handler_t *mh,
                                 const flux_msg_t *msg,
                                 void *args)
{
...
void node_manager_enable_disable_dynamic (bool flag)
{
    ...
    fft_enable = flag;
    log_message (
        "Got disable/enable for dynamic power "
