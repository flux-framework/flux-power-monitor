...
void reset_temp_buffer (char *global_temp_buffer, size_t global_temp_buffer_size)
{
...
void update_temp_buffer (retro_queue_buffer_t *buffer, size_t start_index)
{
    ...
    current_element_monitor =
        retro_queue_buffer_iterate_until_before_tail (buffer,
                                                      &node_power_cmp,
                                                      current_element_monitor,
                                                      &concatenate_csv_callback,
                                                      &global_temp_buffer);
...
void *add_data_to_buffer_thread (void *arg)
{
    ...
    while (!terminate_thread) {
        ...
        if (retro_queue_buffer_marker == 0 && global_file_write) {
            update_temp_buffer (node_power_data->node_power_time,
                                retro_queue_buffer_get_max_size (
...
void power_monitor_init (size_t buffer_size)
{
    ...
    if ((ret = pthread_create (&data_thread,
                               NULL,
                               add_data_to_buffer_thread,
                               node_power_data->node_power_time))
        != 0) {
    ...
    if (global_file_write) {
        ...
        if ((ret = pthread_create (&file_thread, NULL, file_write_thread, NULL)) != 0) {
