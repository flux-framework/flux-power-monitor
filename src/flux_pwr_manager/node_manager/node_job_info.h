...
typedef struct node_job_info {
    ...
    pwr_policy_t
        *node_job_power_mgr[12];  // Right now putting power policy for each job
} node_job_info;
node_job_info *node_job_info_create (uint64_t jobId,
                                     char *job_cwd,
...
void node_job_info_reset_power_data (node_job_info *job_data,
                                     int deviceId,
                                     double powerlimit);
#endif
