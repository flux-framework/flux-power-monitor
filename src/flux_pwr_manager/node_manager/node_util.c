...
void get_header (char **header, node_job_info *data)
{
    ...
    char base_header[] =
        "Timestamp (ms),Node Power (W),Socket 0 CPU Power "
                                           ...
                                           node_job_info *job_data)
{
    if (data == NULL || full_node_data == NULL || job_data == NULL)
        return -1;
    int marker = 0;
    marker = snprintf (scratch_buffer,
                       MAX_CSV_SIZE - 1,
                       "%ld,%lf",
                       full_node_data->timestamp,
                       full_node_data->node_power);
    for (int i = 0; i < NUM_OF_CPUS; i++) {
        marker += snprintf (scratch_buffer + marker,
                            MAX_CSV_SIZE - 1,
                            ",%lf",
...
void sanitize_path (char *path)
{
    ...
    if (len > 1 && path[len - 1] == '/') {  // Check if len > 1 to preserve root "/"
...
void update_device_info_from_json_node (json_t *json,
