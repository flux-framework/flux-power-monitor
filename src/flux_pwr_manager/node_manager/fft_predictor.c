...
void perform_fft (fftw_complex *data,
                  size_t data_size,
                  double *result_first,
                  ...
                  int GPUID)
{
    ...
    fftw_plan p = fftw_plan_dft_1d (data_size, data, out, FFTW_FORWARD, FFTW_ESTIMATE);
    fftw_execute (p);
    ...
    double dominant_frequency = ((double)idx) * ((SAMPLING_RATE * 1.0) / data_size);
    ...
    *result_first = Titer;
    ...
    log_message ("GPU:%d Titer: %f seconds,fft_size %ld \n", GPUID, Titer, data_size);
    *result_second = second_Titer;
    // log_message("Second Dominant Frequency: %f Hz, Titer: %f seconds and fft_size
    // %d\n",
            ...
            if (wait_result == ETIMEDOUT) {
                for (int i = 0; i < NUM_OF_GPUS; i++) {
                    if (fft_job_data[i]->jobId != 0) {
                        ...
                        if (data->cumulative_sum + (FFT_SAMPLING_PERIOD * SAMPLING_RATE)
                            >= FFT_BUFFER_SIZE) {
                            data->data_copied_count = 0;
                            data->fft_input_index = 0;
                            data->cumulative_sum = 0;
                            memset (data->device_data,
                                    0,
                        ...
                        data->buffer_marker_list = retro_queue_buffer_iterate_from (
                            ...
                            &copy_node_power_to_fftw_callback,
                            (void *)data,
                            FFT_SAMPLING_PERIOD);
                        ...
                        *fft_size = data->cumulative_sum;
                        ...
                        retro_queue_buffer_push (fft_result_array->gpus_period[i],
                                                 result);
                        retro_queue_buffer_push (fft_result_array
                                                     ->gpus_period_secondary[i],
                                                 result_second);
                        retro_queue_buffer_push (fft_result_array->fft_size[i],
                                                 fft_size);
                        double average =
                            do_average (fft_result_array->average_period_tracker[i]);

                        retro_queue_buffer_pop (
                            fft_result_array->average_period_tracker[i]);
                        double *avg = malloc (sizeof (double));
                        *avg = average;
                        retro_queue_buffer_push (fft_result_array
                                                     ->average_period_tracker[i],
                                                 avg);
                        ...
                        // Set up the next time to check the buffer (in another 30
                        // seconds)
                        clock_gettime (CLOCK_REALTIME, &wait_time);
                        wait_time.tv_sec += (int)FFT_SAMPLING_PERIOD / SAMPLING_RATE;
                    }
...
void new_fft_result (int i)
{
    ...
    fft_result_array->gpus_period_secondary[i] = retro_queue_buffer_new (100, free);
    fft_result_array->fft_size[i] = retro_queue_buffer_new (100, free);
    fft_result_array->average_period_tracker[i] = retro_queue_buffer_new (100, free);
}
