...
int update_node_powerlimit_rpc (job_mgr_t *job_mgr, flux_t *h, double *power_data)
{
    int local_rank = 0;
    for (int i = 0; i < job_mgr->num_of_nodes; i++) {
        ...
        log_message ("sending new power data for node %s and local rank %d",
                     job_mgr->node_hostname_list[i],
                     local_rank);

        if (local_rank == 0) {
            for (int k = 0; k < current_device->num_of_gpus; k++)
                node_manager_set_powerlimit (job_mgr->jobId,
...
int send_new_job_rpc (flux_t *h, job_mgr_t *job_mgr)
{
    ...
    job_mgr->job_pwr_policy->get_power_dist (job_mgr->num_of_gpus,
                                             job_mgr->device_pwr_stats,
                                             job_mgr->powerlimit,
                                             job_mgr->powerlimit,
                                             power_data);
    for (int i = 0; i < job_mgr->num_of_nodes; i++) {
        ...
        if (local_rank == 0) {
            node_manager_new_job (job_mgr->jobId,
                                  job_mgr->cwd,
                                  job_mgr->job_name,
                                  current_device);
            for (int k = power_offset; k < power_offset + current_device->num_of_gpus;
                 k++) {
                if (node_manager_set_powerlimit (job_mgr->jobId,
                                                 power_data[i],
                                                 current_device
                                                     ->device_id_gpus[k - power_offset])
                    < 0)
            ...
            log_message (
                "checking  sending RPC to rank %d job_mgr "
...
int send_end_job_rpc (flux_t *h, job_mgr_t *job_mgr)
{
    for (int i = 0; i < job_mgr->num_of_nodes; i++) {
        ...
        if (local_rank == 0) {
            ...
            if (flux_rpc_pack (h,
                               "pwr_mgr.nm-end_job",
                               job_mgr->hostname_rank_mapping[i],
                               FLUX_RPC_NORESPONSE,
                               "{s:I}",
                               "jobid",
                               job_mgr->jobId)
