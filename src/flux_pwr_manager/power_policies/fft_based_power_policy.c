...
double get_new_powercap (pwr_policy_t *mgr, double powerlimit, double new_period)
{
    ...
    if (mgr->powercap_history == NULL)
        return new_powercap;
    size_t current_size = retro_queue_buffer_get_current_size (mgr->powercap_history);
 ...
 *
 * */
double fft_get_powercap (pwr_policy_t *mgr,
                         double powerlimit,
                         double current_powercap,
                         void *data)
{
    if (!data)
        return powerlimit;
    ...
    *period = new_period;
    ...
    file_logger_add_data_to_buffer (mgr->file_log,
                                    j_key,
                                    strlen (j_key),
                                    j_data,
                                    strlen (j_data));
...
double fft_get_power_dist (int num_of_elements,
                           pwr_stats_t *stats,
                           double pwr_avail,
                           double pwr_used,
                           double *result)
