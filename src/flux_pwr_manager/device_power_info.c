...
device_power_profile *device_power_profile_new(device_type type,
  ...
  device_data->device_power_history = circular_buffer_new(buffer_size, free);
  if (device_data->device_power_history == NULL) {
    free(device_data);
...
void device_power_profile_destroy(device_power_profile *device_data) {
  if (device_data == NULL)
    return;
  if (device_data->device_power_history != NULL)
    circular_buffer_destroy(device_data->device_power_history);
}
int device_power_profile_add_power_data_to_device_history(
    device_power_profile *device_data, power_data *data) {
  double agg = do_agg(device_data->device_power_history, data->power_value,
