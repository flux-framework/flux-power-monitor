...
job_data *job_data_new(uint64_t jobId, char **node_hostname_list,
  ...
  newJob->jobId = jobId;
  newJob->job_power_history = circular_buffer_new(POWER_HISTORY_SIZE, free);
  if (newJob->job_power_history == NULL) {
  ...
  if (newJob->node_hostname_list == NULL) {
    ...
    circular_buffer_destroy(newJob->job_power_history);
    free(newJob);
    return NULL;
  }
...
void job_data_destroy(job_data *job) {
  ...
  circular_buffer_destroy(job->job_power_history);
  job->job_power_history = NULL;
