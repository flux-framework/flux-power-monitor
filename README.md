...
Flux Power Monitor is a lightweight telemetry module for the Flux resource manager designed to provide job-level power statistics. It aggregates power data over time for each node and its respective hardware components based on the provided `jobId`.
...
```
The power data looks like this:

```
(base) (s=2,d=1) [namankul@tioga25 05:24 PM flux-power-monitor ] python scripts/power_query.py -j $(flux job last)
